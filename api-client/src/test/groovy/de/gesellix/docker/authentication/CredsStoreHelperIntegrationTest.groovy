...
class CredsStoreHelperIntegrationTest extends Specification {
  ...
  def "handles errors more or less gracefully"() {
    when:
    CredsStoreHelperResult result = helper.getAuthentication(System.properties['os.name'] == "Linux" ? "secretservice" : "osxkeychain", "foo")
    then:
    result.data == null
    and:
  ...
  def "handles missing docker-credential-helper more or less gracefully"() {
    when:
    CredsStoreHelperResult result = helper.getAuthentication("should-be-missing", "foo")
    then:
    result.data == null
    and:
