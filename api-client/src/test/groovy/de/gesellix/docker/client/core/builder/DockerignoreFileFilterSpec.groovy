package de.gesellix.docker.client.core.builder
...
class DockerignoreFileFilterSpec extends Specification {
  ...
  def "collects desired files"() {
    given:
    def baseDir = getClass().getResource("/dockerignore").file
  ...
  def "collects all non-dockerignored files"() {
    given:
    def baseDir = getClass().getResource("/dockerignore-all-but-some").file
    def base = new File(baseDir)
    when:
    def collectedFiles = new DockerignoreFileFilter(base, []).collectFiles(base)
    then:
  ...
  def "handles trailing slashes in exclude patterns"() {
    given:
    def baseDir = getClass().getResource("/dockerignore_subdirs").file
    def base = new File(baseDir)
    when:
    def collectedFiles = new DockerignoreFileFilter(base, []).collectFiles(base)
    then:
  ...
  def "handles trailing backslashes in patterns (windows only)"() {
    given:
    def baseDir = getClass().getResource("/dockerignore_windows").file
    def base = new File(baseDir)
    when:
    def collectedFiles = new DockerignoreFileFilter(base, []).collectFiles(base)
    then:
